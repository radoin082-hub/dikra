...
function validateAllRankings() {
    ...
    rankingInputs.forEach(function (input, index) {
        ...
        var newRanking = input.value.trim(); // Trim to remove leading/trailing whitespace

        // If the input is empty, skip validation for this input
        if (newRanking === "") {
            console.error("Empty ranking for student ID:", studentId);
            return;
        }

        newRanking = parseInt(newRanking); // Convert to integer

        if (isNaN(newRanking) || newRanking < 1 || newRanking > rankingInputs.length) {
            console.error("Invalid ranking for student ID:", studentId, ", Value:", newRanking);
            return;
        }

        var newAverage = averageInputs[index].value.trim(); // Trim to remove leading/trailing whitespace

        if (isNaN(newAverage) || newAverage < 1 || newAverage > 20) {
            console.error("Invalid average for student ID:", studentId, ", Value:", newAverage);
            return;
        }

        studentData.push({ "student_id": studentId, "new_ranking": newRanking, "new_average": newAverage });

        // Update UI with new ranking and average
        var rankingCell = input.closest('tr').querySelector('td:nth-child(8)');
        rankingCell.innerText = newRanking;

        var averageCell = input.closest('tr').querySelector('td:nth-child(7)');
        averageCell.innerText = newAverage;
    });

    // Send student data to server for updating rankings
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "update_ranking.php", true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onreadystatechange = function () {
